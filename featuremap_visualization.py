...
from MyVGG16_dropout import MyVGG16_dropout
model = MyVGG16_dropout(inputs).CreateMyModel()


# 加载上次结束训练时的权重
model.load_weights(r"doorModels\cp-327-0.048-0.984-1.000-0.485-0.878-0.969.h5")
...
layeroutput = model.get_layer("block5_conv3")
...
with tf.GradientTape() as gtape:
    ...
    print("prob:", prob)
    gradient = gtape.gradient(prob, conv_output)  # 类别与卷积层的梯度 (1,16,16,512)
    print("gradient:", gradient)
    # pooled_grads = keras.backend.mean(gradient, axis=(0, 1, 2))
    pooled_grads1 = tf.reshape(keras.backend.mean(gradient, axis=(1, 2)), [B, 1, 1, 512])
    pooled_grads2 = tf.reshape(keras.backend.max(gradient, axis=(1, 2)), [B, 1, 1, 512])
    pooled_grads = tf.add(pooled_grads1, pooled_grads2)
    # keras.backend.max()
    print("pooled_grads:", pooled_grads)  # 特征层梯度的全局平均代表每个特征层权重
heatmap = tf.reduce_sum(tf.multiply(pooled_grads, conv_output), axis=-1)  # 权重与特征层相乘，512层求和（求和或者求平均都可以不影响）
#  考虑对卷积特征图运用平均池化和最大池化得到响应图
...
thresholdList = [0.4, 0.6, 0.8]
for threshold in thresholdList:
    heatmap = tf.where(heatmap > threshold, heatmap, 0)
    print("heatmap:", heatmap)
